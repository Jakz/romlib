...
public class NoIntroCataloguer implements GameCataloguer
{
  ...
  static
  {
    ...
    mappers.put("United Kingdom", new Rule(game -> game.getLocation().add(Location.UNITED_KINGDOM)));

    mappers.put("Unknown", new Rule(game -> {
    })); // maybe a Location.UNKNOWN should be used?
    ...
    Predicate<Game> versionUntouched = g -> {
      ...
      // if (!correct)
      // throw new IllegalArgumentException("Version should be untouched for
      // "+g.getTitle());
      return correct;
    };
    ...
    mappers.put("Preview Beta",
        new Rule(versionUntouched, game -> game.setVersion(new Version.Custom("Preview Beta"))));
  ...
  public void catalogue(Game game)
  {
    ...
    if (firstParen != -1)
    {
      Arrays.stream(title.substring(firstParen).split("\\(|\\)")).filter(s -> !s.isEmpty()).map(s -> s.trim())
          .forEach(s -> {
            Arrays.stream(s.split(",")).map(t -> t.trim()).filter(t -> !t.isEmpty()).forEach(t -> {
              ...
              // if (game.getLocation().isJust(Location.EUROPE) &&
              // game.getLanguages().empty())
